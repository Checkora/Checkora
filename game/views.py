...
def ai_move(request):
    ...
    if (
        game.generate_fen_key() != before_fen
        or game.current_turn != before_turn
        or game.game_status != before_status
    ):
        logger.error(
...
def hint_move(request):
    ...
    if hint_count >= 3:
        return JsonResponse({"valid": False, "message": "Hint limit reached"})
    ...
    if game.mode == "ai" and game.current_turn != player_color:
        return JsonResponse({"valid": False, "message": "Wait for your turn."})
    ...
    if not best:
        logger.debug(
            "Hint calculation returned no move for current position. game_status=%s draw_reason=%s",
            game.game_status,
            game.draw_reason,
        )
...
def stats_view(request):
    ...
    user_results = GameResult.objects.filter(user=request.user).exclude(
        mode__in=["", None]
    )
