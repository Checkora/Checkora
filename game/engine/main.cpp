...
bool isSquareAttacked(int tr, int tc, string attackerColor) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (inBounds(r, c) && board[r][c] == pKnight)
      return true;
  }
  ...
  if (inBounds(tr + dir, tc - 1) && board[tr + dir][tc - 1] == pPawn)
    return true;
  if (inBounds(tr + dir, tc + 1) && board[tr + dir][tc + 1] == pPawn)
    return true;
  ...
  for (int r = tr - 1; r <= tr + 1; r++)
    for (int c = tc - 1; c <= tc + 1; c++)
      if (inBounds(r, c) && (r != tr || c != tc) && board[r][c] == pKing)
        return true;
...
bool validKing(const string &color, int fr, int fc, int tr, int tc) {
  ...
  if (fr == tr && abs(tc - fc) == 2) {
    if (color == "white" && fr == 7 && fc == 4) {
      if (tc == 6 && W_K_CASTLE && isEmpty(board[7][5]) &&
          isEmpty(board[7][6])) {
      ...
      if (tc == 2 && W_Q_CASTLE && isEmpty(board[7][3]) &&
          isEmpty(board[7][2]) && isEmpty(board[7][1])) {
        if (!isSquareAttacked(7, 4, "black") &&
      ...
      if (tc == 6 && B_K_CASTLE && isEmpty(board[0][5]) &&
          isEmpty(board[0][6])) {
      ...
      if (tc == 2 && B_Q_CASTLE && isEmpty(board[0][3]) &&
          isEmpty(board[0][2]) && isEmpty(board[0][1])) {
        if (!isSquareAttacked(0, 4, "white") &&
...
bool validateMove(const string &turn, int fr, int fc, int tr, int tc,
  ...
  switch (type) {
  case 'p':
    ok = validPawn(turn, fr, fc, tr, tc);
    break;
  case 'r':
    ok = validRook(fr, fc, tr, tc);
    break;
  case 'n':
    ok = validKnight(fr, fc, tr, tc);
    break;
  case 'b':
    ok = validBishop(fr, fc, tr, tc);
    break;
  case 'q':
    ok = validQueen(fr, fc, tr, tc);
    break;
  case 'k':
    ok = validKing(turn, fr, fc, tr, tc);
    break;
  }
  ...
  for (int tr = 0; tr < 8; tr++) {
    for (int tc = 0; tc < 8; tc++) {
      if (validateMove(turn, row, col, tr, tc, true)) {
        ...
        m.tc = tc;
        m.promoPiece =
            isPromotionMove(piece, tr) ? (isWhite(piece) ? 'Q' : 'q') : '\0';
      ...
      for (int tr = 0; tr < 8; tr++) {
        for (int tc = 0; tc < 8; tc++) {
          if (validateMove(side, r, c, tr, tc, true)) {
            ...
            m.tc = tc;
            m.promoPiece =
                isPromotionMove(p, tr) ? (isWhite(p) ? 'Q' : 'q') : '\0';
            moves.push_back(m);
          }
        }
      }
...
void orderMoves(vector<Move> &moves) {
  sort(moves.begin(), moves.end(), [](const Move &a, const Move &b) {
    ...
    if (!isEmpty(board[a.tr][a.tc]))
      sa += pieceValue(board[a.tr][a.tc]) + 1000;
    if (!isEmpty(board[b.tr][b.tc]))
      sb += pieceValue(board[b.tr][b.tc]) + 1000;
...
bool leavesKingInCheck(const Move &m, const string &side) {
  ...
  if (tolower(static_cast<unsigned char>(p)) == 'p' && m.fc != m.tc &&
      board[m.tr][m.tc] == '.') {
    board[m.fr][m.tc] = '.';
  }
  ...
  bool inCheck = (kpos.first != -1) &&
                 isSquareAttacked(kpos.first, kpos.second,
...
int minimax(int depth, int alpha, int beta, bool maximizing) {
  ...
  if (maximizing) {
    ...
    for (auto &m : legal) {
      ...
      bool old_wk = W_K_CASTLE, old_wq = W_Q_CASTLE, old_bk = B_K_CASTLE,
           old_bq = B_Q_CASTLE;
      if (src == 'K') {
        W_K_CASTLE = false;
        W_Q_CASTLE = false;
      ...
      if (src == 'R') {
        if (m.fr == 7 && m.fc == 0)
          W_Q_CASTLE = false;
        else if (m.fr == 7 && m.fc == 7)
          W_K_CASTLE = false;
      }
      if (src == 'r') {
        if (m.fr == 0 && m.fc == 0)
          B_Q_CASTLE = false;
        else if (m.fr == 0 && m.fc == 7)
          B_K_CASTLE = false;
      }
      if (dst == 'R') {
        if (m.tr == 7 && m.tc == 0)
          W_Q_CASTLE = false;
        else if (m.tr == 7 && m.tc == 7)
          W_K_CASTLE = false;
      }
      if (dst == 'r') {
        if (m.tr == 0 && m.tc == 0)
          B_Q_CASTLE = false;
        else if (m.tr == 0 && m.tc == 7)
          B_K_CASTLE = false;
      }
...
void handleStatus(const string &turn) {
  ...
  bool inCheck =
      (kpos.first >= 0) && isSquareAttacked(kpos.first, kpos.second, opponent);
  ...
  if (!hasLegal) {
    if (inCheck)
      cout << "STATUS CHECKMATE" << endl;
    else
      cout << "STATUS STALEMATE" << endl;
  } else {
    if (inCheck)
...
void handleNotation(const string &turn, int fr, int fc, int tr, int tc) {
  ...
  if (type == 'k') {
    if (abs(tc - fc) == 2) {
      if (tc == 6) {
        cout << "NOTATION O-O" << endl;
        return;
      }
      if (tc == 2) {
        cout << "NOTATION O-O-O" << endl;
        return;
      }
    }
  }
  ...
  string res = "";
  if (type == 'p') {
    // Diagonal move for a pawn is always a capture (handles future En Passant
    // support)
    if (fc != tc) {
    ...
    // Disambiguation: Check if other pieces of the same type can move to the
    // same square
  ...
  // Use the piece type from board for status check (ignore promotion choice for
  // now)
  board[tr][tc] = src;
  board[fr][fc] = '.';
...
void handleBestMove(const string &turn, int depth) {
  ...
  cout << "BESTMOVE " << best.fr << " " << best.fc << " " << best.tr << " "
       << best.tc << endl;
}
