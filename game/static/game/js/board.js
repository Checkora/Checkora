(() => {
  ...
  const pKey = (p) =>
    p ? (p === p.toUpperCase() ? "w" : "b") + p.toLowerCase() : null;
  const pColor = (p) =>
    p ? (p === p.toUpperCase() ? "white" : "black") : null;
  const sq = (r, c) => {
  ...
  function updatePlayerNames(data) {
    ...
    if (gameMode === "ai") {
      if (whiteYouTag)
        whiteYouTag.style.display = playerColor === "white" ? "inline" : "none";
      if (blackYouTag)
        blackYouTag.style.display = playerColor === "black" ? "inline" : "none";
    } else {
  ...
  function updateLabels() {
    ...
    if (rLabels)
      rLabels.innerHTML = ranks.map((r) => `<span>${r}</span>`).join("");
    if (fLabels)
      fLabels.innerHTML = files.map((f) => `<span>${f}</span>`).join("");
  }
  ...
  function markPlayable() {
    boardEl.querySelectorAll(".piece").forEach((img) => {
      ...
      const p = board[r][c];
      const isPlayable =
        p && pColor(p) === turn && !(gameMode === "ai" && turn !== playerColor);
  ...
  function refreshHighlights() {
    boardEl.querySelectorAll(".square").forEach((el) => {
      ...
      el.querySelectorAll(".move-dot, .capture-ring").forEach((n) =>
        n.remove(),
      );
    });
    ...
    try {
      ...
      if (data.valid) {
        ...
        if (handleGameStatus(data.game_status, data.draw_reason)) {
          ...
          showStatus(
            turn === "white" ? "White is in check!" : "Black is in check!",
            true,
          );
        } else {
          showStatus("", false);
  ...
  async function requestAIMove() {
    ...
    try {
      ...
      if (data.valid) {
        ...
        turn = data.current_turn;
        lastMove = {
          from: [mv.from_row, mv.from_col],
          to: [mv.to_row, mv.to_col],
        };
        whiteTime = data.white_time;
        blackTime = data.black_time;
  ...
  async function onClick(r, c) {
    ...
    if (selected) {
      ...
      if (board[r][c] && pColor(board[r][c]) === turn) return selectPiece(r, c);
      return deselect();
    }
    selectPiece(r, c);
  }
  ...
  function updateCaptured(cap) {
    ...
    let whitePoints = cap.white.reduce(
      (sum, p) => sum + (point_vals[p.toLowerCase()] || 0),
      0,
    );
    let blackPoints = cap.black.reduce(
      (sum, p) => sum + (point_vals[p.toLowerCase()] || 0),
      0,
    );

    cap.white.forEach((p) => {
      wCapEl.innerHTML += `<img src="${
        PIECE_IMG[pKey(p)]
      }" class="captured-img">`;
    });
    cap.black.forEach((p) => {
      bCapEl.innerHTML += `<img src="${
        PIECE_IMG[pKey(p)]
      }" class="captured-img">`;
    });
  ...
  function renderClocks() {
    ...
    if (gameMode === "ai") {
      ...
      if (playerTimeEl)
        playerTimeEl.textContent = formatTime(
    ...
    if (wYou)
      wYou.style.display =
        gameMode === "ai" && playerColor === "white" ? "inline" : "none";
    if (bYou)
      bYou.style.display =
        gameMode === "ai" && playerColor === "black" ? "inline" : "none";
  }
  ...
  if (autoFlipBtn)
    autoFlipBtn.onclick = () => {
      ...
      autoFlipBtn.style.background = autoFlip
        ? "linear-gradient(135deg, #40c0f0, #2080d4)"
        : "";
  ...
  if (gameOverStartBtn)
    gameOverStartBtn.onclick = () => {
      ...
      const confettiContainer = gameOverOverlay.querySelector(
        ".confetti-container",
      );
      if (confettiContainer) {
        confettiContainer.remove();
