(() => {
    ...
    let activeAI = 'stockfish';
    let stockfishDifficulty = 10;
    ...
    const nameInputs = document.getElementById('nameInputs'); // Wrapped in try/catch later if missing
    ...
    async function applyStockfishMove(bestMove) {
        if (!bestMove || bestMove === "(none)") {
            showStatus("AI has no valid moves.", true);
            return;
        }

        const files = { 'a': 0, 'b': 1, 'c': 2, 'd': 3, 'e': 4, 'f': 5, 'g': 6, 'h': 7 };
        const fc = files[bestMove[0]];
        const fr = 8 - parseInt(bestMove[1]);
        const tc = files[bestMove[2]];
        const tr = 8 - parseInt(bestMove[3]);

        let promoPiece = null;
        if (bestMove.length > 4) {
            promoPiece = bestMove[4];
        }

        await executeMove(fr, fc, tr, tc, promoPiece);
    }
    ...
    async function requestAIMove() {
        ...
        if (activeAI === 'stockfish' && stockfishWorker) {
            const currentFen = generateFEN();
            stockfishWorker.postMessage("ucinewgame");
            stockfishWorker.postMessage("position fen " + currentFen);
            stockfishWorker.postMessage("go depth " + stockfishDifficulty);
        } else {
            try {
    ...
    function requestNewGame(mode) {
        ...
        showConfirm(
            ...
            () => {
                const diffSelect = document.getElementById('confirmDifficultySelect');
    ...
    async function startNewGame(mode, pColor = 'white', difficulty = 'medium') {
        ...
        const wName = wInput ? wInput.value : 'White';
        const bName = bInput ? bInput.value : 'Black';
    ...
    if (welcomeAIBtn) welcomeAIBtn.onclick = () => {
        ...
        if (pveOptions) pveOptions.style.display = 'flex';
    };

    if (backToModes) backToModes.onclick = () => {
        if (pveOptions) pveOptions.style.display = 'none';
    ...
    if (startAIBtn) startAIBtn.onclick = () => {
        const diffSelect = document.getElementById('welcomeDifficultySelect');
        const diff = diffSelect ? diffSelect.value : 'medium';
    ...
    if (gameOverStartBtn) gameOverStartBtn.onclick = () => {
        ...
        const diffSelect = document.getElementById('goDifficultySelect');
        const diff = diffSelect ? diffSelect.value : 'medium';
        gameOverOverlay.classList.remove('active');
        startNewGame(mode, 'white', diff); // Assuming white by default on game over restart
    };
